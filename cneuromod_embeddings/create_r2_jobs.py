...
for model_path in model_paths:
    ...
    for s in subjects:
        if "sub-"+s not in model_path:
            sub = sub+s+","
    sub = sub[:-1]
    cmds.append("""python cneuromod_embeddings/compute_r2.py -m {m} -d {d} -s {s} -t {t} -g {g} -tag {tag} -b {b}\n""".format(
...
#            cmds.append("""python cneuromod_embeddings/compute_r2.py -m {m} -d {d} -s {s} -t {t} -g {g} -a {a} -o {o} -b {b}\n""".format(
...
with open("cneuromod_embeddings/r2_jobs.sh", "w") as r2_job:
    for c in cmds:
        r2_job.write(c)

with open("cneuromod_embeddings/r2_submit_jobs.sh", "r") as r2_sub:
...
with open("cneuromod_embeddings/r2_submit_jobs.sh", "w") as r2_sub:

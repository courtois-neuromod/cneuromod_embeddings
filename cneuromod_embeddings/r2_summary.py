...
def _load_mask(subject, root_data, type_mask, fwhm, cluster, state, xp_type):
    model, mask_img = load_dypac(
        subject=subject, root_data=root_data, fwhm=fwhm, cluster=cluster, state=state, xp_type=xp_type
    )
...
def _r2_inter(root_data, n_subject=6, fwhm=5, cluster=300, state=900, type_mask='cortex', xp_type='friends-s01'):
    ...
    for sub in list_subject:
        print(sub)
        mask = _load_mask(subject=sub, root_data=root_data,type_mask=type_mask, fwhm=fwhm, cluster=cluster, state=state, xp_type=xp_type)

        # inter-subject R2 maps
        hdf5_file = load_r2_inter(subject=sub, root_data=root_data, fwhm=fwhm, cluster=cluster, state=state, xp_type=xp_type)
...
def _r2_intra(root_data, n_subject=6, fwhm=5, cluster=300, state=900, type_mask='cortex', xp_type='friends-s01'):
    list_subject = subject_keys(n_subject)
    val = np.array([])
    all_sub = np.array([])
    all_label = np.array([])
    ...
    for sub in list_subject:
        ...
        hdf5_file = load_r2_intra(subject=sub, root_data=root_data, fwhm=fwhm, cluster=cluster, state=state, xp_type=xp_type)
...
def _r2_other(root_data, atlas, n_subject=6, fwhm=5, type_mask='cortex', xp_type='friends-s01'):
    list_subject = subject_keys(n_subject)
    val = np.array([])
    all_sub = np.array([])
    all_label = np.array([])
    for sub in list_subject:
        print(sub)
        mask = _load_mask(subject=sub, root_data=root_data,type_mask=type_mask, fwhm=fwhm, cluster=20, state=60, xp_type=xp_type)
        hdf5_file = load_r2_other(atlas=atlas, root_data=root_data, fwhm=fwhm, xp_type=xp_type)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--xp_type", type=str, default='friends-s01', help="The type of experiment [friends-s01 (default), friends-s01_clean, friends-s01_clean_multi_fwhm].")

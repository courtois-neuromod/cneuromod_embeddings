...
def _load_components(subject, state, xp_type):
    ...
    bg_img = os.path.join(path_data, xp_type, f"{subject}_space-MNI152NLin2009cAsym_desc-preproc_T1w.nii.gz")
    parcels = os.path.join(path_data, xp_type, f"{subject}_space-MNI152NLin2009cAsym_desc-dypac{state}_components.nii.gz")
    mask = os.path.join(path_data, xp_type, f"{subject}_space-MNI152NLin2009cAsym_label-GM_mask.nii.gz")
...
def _components2surface(components):
    ...
    gii_lh, gii_rh = mni152_to_fslr(masker.inverse_transform(np.sum(components, axis=0)))

    # create surface plot
    plot_surface = Plot(lh, rh)
    plot_surface.add_layer({'left': sulc_lh, 'right': sulc_rh}, cmap='binary_r', cbar=False)
    plot_surface.add_layer({'left': gii_lh, 'right': gii_rh}, cmap='turbo',
                color_range=(0, 5))
    return plot_surface
...
def generate_n_states_maps(subject, state, xp_type):
    ...
    file_fig =  os.path.join(path_fig, f"{subject}_space-MNI152NLin2009cAsym_desc-dypac{state}_components.nii.gz")
    fig.savefig(file_fig)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--xp_type", help="The type of experiment [friends-s02 (default), movie10, hcptrt].")
